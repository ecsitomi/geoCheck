...
{{"summary": "részletes összefoglaló szöveg", "recommendations": "konkrét javaslatok szövege"}}"""
            ...
            # OpenAI API hívás - JAVÍTOTT JSON PARSING
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {
                        "role": "system",
                        "content": """Te egy GEO (Generative Engine Optimization) szakértő vagy. 
KRITIKUSAN FONTOS: A válaszod KIZÁRÓLAG valid JSON formátumban legyen, semmi más!
Ne használj markdown formázást, csak tiszta JSON-t adj vissza."""
                    },
                    {
                        "role": "user", 
                        "content": f"""Elemezd ezt a GEO audit eredményt:

{json_str}

Készíts részletes elemzést és javaslatokat.

ÖSSZEFOGLALÓ szempontjai:
- AI Readiness Score értékelése (átlag: {analysis_summary.get('overview', {}).get('avg_ai_score', 0)})
- Főbb problémák számszerűen
- Platform teljesítmény elemzése
- Kritikus hiányosságok
- Pozitívumok kiemelése

JAVASLATOK szempontjai:
- Top 5 sürgős teendő konkrét lépésekkel
- Platform-specifikus optimalizációk (ChatGPT, Claude, Gemini, Bing)
- Quick wins (gyors eredmények)
- Várható AI Score javulás becslése

KRITIKUS: Válaszolj CSAK ÉS KIZÁRÓLAG ebben a JSON formátumban, semmi mást ne írj:
{{
    "summary": "Itt a részletes összefoglaló szöveg, használj konkrét számokat és adatokat",
    "recommendations": "Itt a konkrét javaslatok szövege, prioritizálva és lépésekre bontva"
}}"""
                    }
                ],
                temperature=0.7,
    ...
    def _parse_ai_response_manually(self, response: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if summary and len(summary) > 50:
                ...
                # Ha túl rövid, használjuk a fallback metódust
                logger.warning("Manuális parsing nem adott megfelelő eredményt, fallback használata")
                # Itt nem hívhatjuk meg közvetlenül a fallback-et mert nincs analysis_summary
                # Helyette egy általános hibaüzenetet adunk
                summary = response[:1500] if len(response) > 1500 else response
                recommendations = "A javaslatok automatikus generálása sikertelen. Kérlek tekintsd át manuálisan az elemzési eredményeket és készíts optimalizálási tervet."
            
            return (
                summary.strip() if summary else "Az összefoglaló generálása részben sikertelen.",
                recommendations.strip() if recommendations else "A javaslatok generálása részben sikertelen."
            )
